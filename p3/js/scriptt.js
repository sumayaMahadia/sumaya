        ...
        success: function(data) {
            ...
            let wikiHtml = `
                <div class="wiki-extract">
                    ${pageData.thumbnail ? 
                        `<img src="${pageData.thumbnail.source}" 
                              alt="${countryName}" 
                              style="float: right; margin: 0 0 15px 15px; max-width: 300px; border-radius: 8px;">` 
                        : ''
                    }
                    <p>${pageData.extract || 'Information about this country is being updated.'}</p>
                </div>
            `;
            $('#wikiContent').html(wikiHtml);
            fetchWikiImages(countryName);
        }
    });
}
